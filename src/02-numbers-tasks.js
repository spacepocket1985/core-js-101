...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorModulusFirst = Math.sqrt(x1 * x1 + y1 * y1);
  const vectorModulusDecond = Math.sqrt(x2 * x2 + y2 * y2);

  const angle = Math.acos(scalarProduct / (vectorModulusFirst * vectorModulusDecond));
