...
function getPolynom(...args) {
  ...
  return function f(x) {
    let formula = 0;
    for (let i = argsArr.length - 1; i >= 0; i -= 1) {
      formula += argsArr[i] * (x ** i);
    }
 ...
 *   partialUsingArguments(fn, 'a','b','c','d')() => 'abcd'
 */
function partialUsingArguments(fn, ...argsMain) {
  return function f(...argsSub) {
    return `${argsMain.join('')}${argsSub.join('')}`;
  };
}
